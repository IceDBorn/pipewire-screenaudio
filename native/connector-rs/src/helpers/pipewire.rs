...
pub fn create_virtual_source(node_name: &String) -> i64 { // TODO Result<i64,String>
  let result = Command::new("pw-cli")
    .arg("create-node")
    .arg("adapter")
    .arg(format!("{{ factory.name=support.null-audio-sink node.name={} node.description={} media.class=Audio/Source/Virtual object.linger=1 audio.position=[FL,FR] }}", node_name, node_name))
    .output();
