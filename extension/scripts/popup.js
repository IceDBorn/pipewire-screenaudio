...
dropdown.addEventListener("change", () => {
	...
	chrome.runtime.sendNativeMessage(MESSAGE_NAME, {
		cmd: "SetSharingNode",
		args: [{ node: selectedNode, micId }],
	});
});
...
async function isRunning() {
	...
	const { isRunning } = await chrome.runtime.sendNativeMessage(MESSAGE_NAME, {
		cmd: "IsPipewireScreenAudioRunning",
		args: [{ micId }],
	});
...
async function updateGui() {
	if (await isRunning()) {
		...
		if (shareStopBtnState !== "stop") setButtonToStop();
	} else if (dropdown.children.length) {
		message.hidden = true;
		dropdown.hidden = false;
		shareStopBtn.hidden = false;
		if (shareStopBtnState !== "share") setButtonToShare();
	} else {
...
async function populateNodesList(response) {
	if (JSON.stringify(response) !== window.localStorage.getItem("nodesList")) {
		...
		option.innerText = "All Desktop Audio";
		option.value = -1;
		...
		if (
			dropdown.innerHTML.indexOf(
				'value="' + window.localStorage.getItem("selectedNode") + '"',
			) > -1
		) {
...
function onResponse(response) {
	...
	chrome.runtime
		.sendNativeMessage(MESSAGE_NAME, { cmd: "GetNodes", args: [] })
		.then(onReload, onError);

	nodesLoop = setInterval(() => {
		chrome.runtime
			.sendNativeMessage(MESSAGE_NAME, { cmd: "GetNodes", args: [] })
			.then(onReload, onError);
	}, 1000);
